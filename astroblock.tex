...
The Kardashev astrophysical law is one of energy consumption versus time for radio-capable civilizations. We take Global Human Power production data and compare it to two Kardashev models. The standard Kardashev one-percent monotonically increasing model that is approximately exponential, and, a linear model that is the OLS fitted linear function to the data. Different statistical properties are inherent in the two competing models considered. The linear model has slope and intercept of 0.58 TW/yr and -1136 years respectively with low scatter about the points to the line. The axis variables for both models have covariance with time, and real Global Power production must have been greater than zero even before the advent of radio technology so caution must be had applying the model before 1964. However, the linear model that covers nearly two orders of magnitude in years and includes several technological eras (and political regulation) of energy production from Coal, Oil, Nuclear to Renuables (Hydro, wind, Geothermal, Solar etc.), as a model is shown to be more consistent with the maximal likelihood function of the Kardashev law for the data. Fundamentally, the linear model is not constrained by the \textbf{memoryless} probability property implicit in constant percentage increase models that preclude negative year-on-year growth and critically underestimate the $\Delta_{\textrm{type ii}}$. In leu of the limitation of the exponential model and the discrepancy of the linear model that radio-capable civilizations may require more than a Hubble time to evolve to a Kardashev type-ii, we therefore select our Kardashev OLS linear model fit and choose to re-normalize it by the Hash-Rate of Bitcoin defining a new Kardashev-Nakamoto model. The re-normalization is done by dividing the Power Production metric (J/s) by the Scalar Bitcoin Hash-Rate function (Hash/s). Incidentally the choice of sound money and power may maximize the Drake equation's longevity term for radio-capable civilizations arguably a function of the standard Kardashev Law that may be justified by the advanced hour of the Doomsday clock. The equivalence unit of J/Hash that results might go in that vein, to satisfy an implicit requirement of the original Kardashev law that includes information-richness, obviating an additional parameter. The Bitcoin hash-rate is a real valued scalar function with two-week difficulty adjustment, so the re-normalization may represent a more realistic Universal model for the Kardashev law. A caveat to this analysis: the energy consumption data employed is the global yearly average total Power produced; nevertheless the Power production metric is an upper limit to Power consumption and so is a valid proxy to refine our straw-person model that provides a somewhat theoretical basis of an interoperability development driver for Astronomy \& Astrophysics; Education, Research and Development, based on Public Open Blockchain Ledger technology. 
...
The distribution of differences has a measured skewness of -0.31 \cite{zw00}, a mean of 2.34 and standard deviation of 2.02. The residual and relative residual of the data from the linear model are shown in Figs: \ref{fig:kardashev1_res} and \ref{fig:kardashev1_rel_res}, respectively and histograms of the distribution of residuals and relative residules shown in Figs. \ref{fig:his_res}, and \ref{fig:his_rel_res}, respectively.
