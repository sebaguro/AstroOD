...
The Kardashev conjecture or hypothetical astrophysical law is one of energy consumption with time (or Power VS Time) for radio-capable civilizations. We take global human Power production data from 1964 to 2022 and compare it to two Kardashev models. The standard "one-percent" per year monotonically increasing or approximately exponential model, and, the Ordinary Least Squares fit linear data-model. The two competing models have significant different mathematical and statistical properties. The OLS linear data-model has a slope and intercept of $5.78 \pm 0.09$ TW per day and $-1.40  \pm 0.02 \times 10^{7} $ TW respectively, and the data-fit is uncanny! However, perhaps more surprisingly is the fact that the standard "one-percent" model visibly diverges from the data! The dimensional variables of power and time have time covariance and may naturally require further restriction; nothwithstanding this and against Julian Day as duly justified we show the linear data-model with its low dispersion across nearly two orders of magnitude in years, that transcends several technological and political eras of modern energy production from coal, oil, nuclear to renuables (hydro, wind, geothermal, solar etc.) has a Negative Log-Likelihood significantly smaller for the same number of parameters as has the standard divergant model. The hypothesis is: that the OLS data-model is not constrained by the \textbf{memoryless} probability property that forces the standard model to diverge by precluding negative year-over-year growth, and to critically underestimate the lapse of time required for a typical nascent radio capable civilization to evolve to a Kard$\,_{\textrm{type-ii}}$ of the original scale, of "$4 \times 10^{33}$ erg/s" or about $4 \times 10^{26}$ J/s or equal to the bolemtric power of a dwarf main sequence star like The Sun. Consequently, in lieu of the conclusion: that the original Kardashev model is not tenable and, to resolve the shortcoming of the linear data-model prediction; that radio-nascent civilizations may require more than a Hubble time to evolve to Kard$\,_{\textrm{type-ii}}$; is that we propose the Kardashev-Nakamoto model as a solution to this conundrum by first renormalizing the Power Production metric (in units: J/s) by the Scalar Bitcoin Hash-Rate function year average (in units: Hash/s). The equivalence unit of J/Hash that results satisfies Kardashev's insight, to condition it (The Kardashev Relation) to account for information-richness without requiring additional parameters. Since the Bitcoin hash-rate is not a monotonically increasing function, with two-week difficulty adjustment then the the memory probability conservation property constraint is automatically met. We speculate on a prediction: if the Kardashev-Nakamoto model reflects physical reality, a handle on the Drake equation's longevity term for 'advanced' civilizations could be better modelled as well as the evolution of the Doomsday clock and the Fermi Paradox. The new Kardashev-Nakamoto straw-person model and scale if approximately true provides a somewhat theoretical basis of an interoperability development driver for Astronomy \& Astrophysics; Education, Research and Development, based on Public Open Blockchain Ledger technology. A caveat to this analysis: the Power metric used is not derived from the "total global yearly average Energy consumed" but "total global yearly average Energy produced"; so for this purpose, production is an upper limit proxy to consumption in this Kardashev-Nakamoto model. 
...
This white paper (WP) examines Public Open Blockchain (OB) ledger technology as a value optimization and interoperability agent for Astronomy Open Development (AOD). A comprehensive review of OB ledger technology: the evolution of protocols, key ideas \& primitives that lead to the so-called Nakamoto consensus effectively solving the Byzantine Generals problem defined by \cite{Lamport1982TheBG} is treated in  \cite{arvindandclark2017} and references therein to define value. In Sec. \ref{sec:intro} of this WP a connection is made between OB to the possible evolution of astrophysical discoveries and computer data-science systems that co-evolve with astronomy and astrophysics, hereafter astronomy. At the very least, if OB technology is indeed a nascent corner-stone disruptive technology it seems reasonable for it to be considered for the Open Decentralized Development of Astronomy, as expanded on in Sec. \ref{sec:bc_review} and through-out this WP.  OB technology may be visualized schematically in figure 1 of \cite{arvindandclark2017} that via the Nakamoto consensus includes DESCI and DEFI, two key ideas that may seem implicit but made explicit in this WP are added into 
Fig. \ref{fig:narayanan} (by adapting their figure). In this WP an inexhaustive list of other native blockchains and protocols are also selected based on sustained marked-cap, history and potential of use for AOD. An astronomy token for AOD is also not ruled out nor the idea much entertained, but such a token could be pre-mined and distributed to IVOA, IAU, Astropy and other identified community members where astrophysical education and research are conducted. In Sec. \ref{sec:energy} the heart of the paper a connection between OB and astronomy is made via the Kardashev conjecture, originally one of the global energy rate of consumption vs time, for all civilizations with radio communication capability. We show with energy data for The Earth that the standard Kardashev "one-percent" model when Power production is taken as an upper-limit proxy to consumption must be ruled out. Constant percentage increase models have the form $(1+x)^{t}$, for $x<< 1$, and include the standard one-percent Kardashev model as originally proposed by \cite{kar64} and by later studies, for example:  \cite{sagan73} and \cite{gray2020}. Monotonically increasing models diverge from data with gaussian variance because restricting the year-over-year difference distribution say, of a Power metric, to positive real values violates a memory conservation property of its probability distribution, that as argued must also be a sub-optimal condition to a civilization's longevity: \textit{the hypothesis}, and, \textit{the conundrum} is that given the hypothesis, that the standard "one percent" Kardashev model diverges from the data, the alternate linear ordinary least squares fitted data-model although has remarkably low dispersion over six decades of time about the data, it itself as a model appears unfathomable since it predicts a Kard$\,_{\textrm{type-ii}}$ time-scale (from 1964) of $\sim 10^{11} \textrm{yr} \gg {1/H_o}$, over an order of magnitude greater than the Hubble time is $1/H{_0}$, where $H_{0}$ is the Hubble constant, for the standard $\Lambda$CDM paradigm. Alas the \textbf{the conundrum}, it may be yet be resolved by a simple re-normalization of the total power production metric by the Bitcoin hash-rate. The decision to re-normalize the Kardashev model to the Kardashev-Nakamoto model by the Bitcoin hash-rate would appear to fulfill the requirement of information-richness, necessary to `Secure an enormous quantity of information', as originally proposed by \cite{kar64}. The consideration for the Drake equation is that the number of `advanced' civilizations in The Galaxy with radio communication capacity depends on the longevity factor $L$ that represents the average existence-time-scale window of Drake's conjecture. This factor is considered as an uncertain term in the Drake equation. So this new metric designed to be predicative provides the Kardashev-Nakamoto model as a means for an uncensurable transaction networked system designed ultimately to be a decentralized medium of exchange, unit of account and store of value to be provided by the Bitcoin network. The timely nature of this WP in 2023 is the same year that the Bulletin of the Atomic Scientists have forward the Doomsday Clock by ten seconds to 23h:58m:30s reaching 90 second to midnight, the closest it has come to midnight than in any other period in recorded Human history and that there is some expectation that with the war in Ukraine it may be further adjusted forward towards midnight in when it is next revised in Jan 2024. Hence the new Kardashev-Nakamoto consensus may reflect a Universal common digital gold standard for AOD and beyond, one the paves the way for innovation through transnational protocols that are based on it that preserve information-richness.  In Sec. \ref{sec:btc4} specific use-cases of OB for AOD are selected and in Sec. 5 and 6 future application prospects and further discussion are respectively considered.
...
Cryptography, the basis of OB ledger technology or Distributed Ledger Technology (DLT) has until now not been much considered for astronomy development as revealed in 'full-text' searches of astronomical peer-reviewed international journal papers, least as a potential interoperability catalyzing agent. In fact cryptography for Astronomy has mainly been explored as an ancillary technology within data transfer applications like data compression, permission, (eg: ssh-rsa) and data fidelity procedures (eg: checksum) for software development.
