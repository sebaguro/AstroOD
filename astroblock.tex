...
This white paper examines Blockchain technology as a value optimization and interoperability agent for Astronomy Open Development (AOD). A comprehensive review of public and open Blockchain ledger technology including the evolution of Blockchain protocols: key ideas \& primitives leading to the  so-called Nakamoto consensus that effectively solves the Byzantine Generals problem defined by \cite{Lamport1982TheBG} is treated in  \cite{arvindandclark2017} and references therein. In Section \ref{sec:intro} a brief introduction is given on the history of the computer and computer systems and an attempt is made to connect The Computer as a technological tool that has co-evolved with advances in astronomy and astrophysics, hereafter astronomy. If Blockchain technology is indeed a nascent corner-stone technology for Astronomy it seems reasonable for it to now be considered for the Open Decentralized Development of Astronomy, as set out in this WP in Section \ref{sec:bc_review}. It is argued herein that the Nakamoto consensus of Blockchain must be at least considered as a fundamental element for the natural evolution of the computer and computing systems in which a sound value system for Astronomy transactions is optimized at least in timescales of a few years. In this section a short review of Blockchain is given based on figure 1 of \cite{arvindandclark2017}) moreover two key ideas: the Decentralized Finance and Decentralized Science Movements (DEFI and DESCI) have additionally been included as a fiducial family of parameters in AOD models, incorporated into 
Fig \ref{fig:narayanan} of this white-paper. In this section other Chains and protocols are identified for historical reasons and as potentially useful for AOD that are also considered. To put our claim of Blockchain in a epistemological context, in Section \ref{sec:energy} we reanalyze the Kardashev relation and show that yearly increase in world consumption for the only cicilization we know using total production as an upper limit, do in fact not follow the $(1+x)^{n}$ as originally thought by Kardashev and depicted in previous studies. To reconcile the Type II timescale we therefore take the Nakamoto consensus and build it into our model by rescaling the Total Energy Production Vs Time function of potential Civilizations using the Bitcoin hash-rate. This decision is made since the original Kardashev and re-fitted Kardashev relations that include newer data do not predict the civilization time-scale type limits as reported and because the Drake equation that determines the amount of advanced technological civilizations contains a limiting term which reflects the window in time for technological development that is sensitive to global issues like war and environment. Therefore the ability to have a common global value standard for transactions for open development is best enshrowed in the evolution of the Nakamoto consensus. Our modified model for the Kardashev scale is therefore re-normalized by the Bitcoin hash-rate and includes the assumption that the Bitcoin (Captial `B') as a network and bitcoin (Lower-case `b') the commodity combined, must represent a sound store-of-value transaction base-layer protocol for AOD one that preserves information-richness as necessarily identified by Kardeshev in his original 64' paper and subsequent works. In Section 4 specific use-cases of Blockchain AOD are identified and examined and finally future prospects and discussion are addressed in Section 5 and Section 6, respectively.
