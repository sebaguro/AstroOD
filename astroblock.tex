...
An interoperability  development driver based on (Open) blockchain legdger technology is considered for Astronomy \& Astrophysics Education, Research and Development from K-12 to tertiary-research level to facilitate Astronomy Open Developement projects in as a system. Initial funding should be bootstraped from the decentralized finance movement or DEFI, given the assumption it is a key idea of the open blockchain stack which appears to be disrupting centralized banking. Justification: (i) Telescope brokers' reliance on federated learning models to effectively extract information from large telescope survey data in the data data-Tsunami era. (ii) Historical Observatory patrimony be revalued. (iii) Hardship for the development of science due to centralized funding. eg: Technology being more difficult to obtain for more heavily devalued national currencies. The challenge then for the astronomical community is to to build a complimentary and sustainable store-of-value and value-added programmable incentive layer upon open blockchain protocol standards within existing governance structures across institutions. Motivation: the demonstrably steep decline in the half-life function of PhD Astronomy majors in public US universities and associated national systems or brain drain capital flight could be sufficient motivation given the resources it takes to produce a PhD major in astronomy, yet other barriers to open development are also examined. This paper proposes a path for on-boarding, and examines open development metrics that could be useful , that must be established by the community, automatically monitored, and suitably refined through bodies like IVOA, all this achieved via blockchain public ledger technology. 
...
This white paper examines blockchain as a value optimization and interoperability agent for Astronomy Open Development (AOD). A comprehensive review paper of public and open blockchain ledger technology including its evolution: key primitives and protocols that lead to the so-called Nakamoto consensus solving the Byzantine Generals problem as defined by Lamport, XXX, and YYY 1972, is treated in  \cite{20d30b4efb014b21b7ab27f5218692ab} (and references therein). In Fig \ref{fig:narayanan} the key idea of the decentralized finance movement is added, simply adapting fig 1 of \cite{20d30b4efb014b21b7ab27f5218692ab}. Blockchain DEFI is identified as a complimentary grant funding source or seed funding AOD agent. As AOD matures it may require yield from DEFI markets and, ultimately the measure of success of blockchain will be determined by its specific use by existing users of existing structures. On this point, Avid Avaryaran notes that the main challenge for wide-spread adoption of decentralized open blockchain ledger technology lies in the utility of use by existing users and institutions rather than in the necessity of creating new blockchain institutions. With this in mind we set out to identify and examine some salient issues that arise given the tight dependence of the nature of astronomy development and data-science through a tight coupling to the The Computer, our model heuristic.

The steeply falling half-life career function \cite{milo_2018} of US PhD Astronomy majors taken as a proxy, for the larger professional market could be sufficient motivation to propose a complimentary development funded system, but so too more specific and wider Astronomy \& astrophysics community issues. Moreover other STEM communities may also find use in this white-paper since Open Source and Open Development standard protocols are unifying and converging over different scientific disciplines. Sufficient motivation for Open Development with blockchain within other fields is similarly proposed; for example [HapMap project and its Open Source dependence for the human Genome Project, \cite{GITTER2008529} to prevent closed patent licensing]
...
\label{sec:intro}
%
 The Antikythera mechanism was discovered in 1901 from the debris of a ship-wreck that were estimated to have occurred about two thousand years earlier. Computer \& Data Scientists on average recognize it to be the first analogue computer mechanism or simply put: The Computer. \citet{Freeth2021} and other studies have demonstrated that the Antikythera mechanism was designed to solve mostly astronomical problems based on a technology that must have rested on much support from ancient Greek Astronomers, including Hipparchus and Archimedes. The detailed scanning and modelling of the relic 'jig-saw puzzle' by \citet{freeth2021} reveal that it contained at-least three finely crafted rotating discs and more than thirty cogs that were designed, forged and crafted with adequate tolerances to calculate and predict the positions of important astronomical sources including The Sun; The Moon; some planets over time. These calculations were necessary for navigation, agriculture, culture and scientific/technological development.
 
 The Ancient Greek astronomers had defined the magnitude logarithmic system and introduced concepts of limits and infinitesimals in geometry that would become the basis of differential calculus and numerical methods, tool-kits that have proved to be of immense importance to the development of astronomy.
 
 It is now known after a plethora of multi-disciplinary studies like \citet{Freeth2021} that during the decline of Ancient Greece this "Computer" was temporily lost to most of the technological world, when only in the fourteen century where comparable `sophisticated' mechanisms developed in Astronomical clock machinery in Europe. 
...
Cryptography is the basis of Blockchain Ledger technology, a technology that has not been much considered yet for astronomy development, least, as a potential interoperabile catalyzing agent. Interestingly cryptography for Astronomy has only been used as a tool for data compression, data permission (rights) and related data fidelity issues.


Conversely,  `Machine-learning' and `Data Mining' Artificial Intelligence (AI) techniques are found in an army of published astronomy studies in peer-reviewed journals. Some studies estimate the physical parameter spaces of sources from large ensembles of data measurements of the observable parameter space, source classification and astrophysical relations or laws that govern The Universe have been constrained and given the so-called data-tsunami AI may even be proving integral to Astronomy development since mega-survey's have requirement that consider Velocity, Volume, Veracity, and Variety; all necessary constraints for the optimization of data-science requirements. High cadence astrophysics events for example through multi-messenger observations are required to study kilonova transients that depend on real-time AI alert systems and have recently revealed from Kilonova specto-photometric modelling that rapid neutron capture, or r-process, nucleosynthesis, requires dense, neutron-rich material as measured by  the ejecta produced by the collision of two neutron stars, see \cite{artola2020}. AI is playing a fundamental role in the evolution of Astronomy, including gravitational wave, radio (eg: SKA) and optical (eg: Vera Rubin). However,a corollary then arises, to unite brokers across different wavelengths and AI learning methods. Could blockchain be thus far a missing link for Astronomy (Open) Development, in an environment where the half-life function of PhD majors in astronomy is exponentially decreasing and projects are becoming much more reliant on team science?  For example, vera Rubin has only enough bandwidth for full data streams to 7 `community Brokers' which have been defined: Alerce, Ampel, Antares, Babamul, Fink, Lasair, Pitt-Google. Some of these brokers are lead by science teams from specialized areas of astrophysics. Could DEFI be used to fund dedicated Astropy community Brokers, rewarded via Oracles for soft-ware and data analysis contributions? DESCI via existing governance structures, like IVOA may be required to mitigate these and other closed development issues. 
...
\label{sec:bc_review}
\cite{20d30b4efb014b21b7ab27f5218692ab} show insurmountable evidence as do many other authors therein that standard open blockchain protocols are much rooted in academic studies. Fig \ref{fig:narayanan} is an adaptation of their fig1 that includes the so-called Nakamoto consensus of 2008 with the addition of the key-idea of the "Decentralized Finance Movement". Also identified is the Automatic Market Making Technology that permits Tx or token swaps without the need of a centralized order book that is to say independent of any third-party. So too, is include UNI, the UNISWAP governance token \href{https://github.com/Uniswap}{UNISWAP}, as an example of a grant source DEFI governence token since it was the first decentralized exchange (DEX) to include automatic market making technology to issue a governance air-drop token, is to date one of the largest DEFI governence tokens whose treasury has more the 3 billion USD in reserve. Note: on the advent of smart-contracts, like those used within the UNI governance protocol, according toto Nick Szabo and probably for AOD they are a blockchain primitive not related to AI but to automatic conditional P2P transactions that do not require a third party mediator and are automatically executed within a dynamic and sound value system like that based on the Nakamoto consensus. 
...
\begin{figure}[h!]
  ...
  \includegraphics[width=0.5\textwidth]{figs/F4.large.jpg}
\end{figure}
The Nakamoto Consensus of blockchain, from the white-paper of \cite{nak2009} and subsequent Bitcoin Improvement Proposals (BIP) may in fact be such a bases for AOD. A review can be found in \citet{Hansen2020Book}. Properties that for its selection as a valid  store of value system include the Nakamoto block-size constraints that allow full-nodes to be run cheaply even on a single PC and the BTC supply function that is capped at 21,0000 is dependent on miner rewards and difficulty adjustments. Moreover, since its inception in 2009, after 12 years of price action (see Fig 1) it has proved to be a valid store-of-value with an ROI far superior to the USD as a standard. Although as noted, on short time-scales, (less than a year) the BTC asset is volatile although with time appears to be in general becoming less volatile and although the BTC/USD pairing is somewhat also correlated to global financial markets (Lyn Alden) and inherent volatility yet on longer time-scales BTC volatility as measured by Cohen et al. and other authors has been measured to be falling. 
