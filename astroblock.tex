...
The Bitcoin network has been often criticized for the power required. The Cambridge University blockchain group in a 2021 looked at this issues and concluded that based on models that take into account the projected Bitcoin hash rate, the evolution of ASICS, required to solve for new blocks, that a as percentage of world total energy for Bitcoin will remain below 0.1 \percent.  More than just the production of new blocks or new bitcoins, it must be recognized that (i) this cost is to secure the network and that, (ii) Energy use and energy production are two separate issues (iii) There has been a significant percentage of POW powered by renewable energy sources, that this could be the rule if so desired via technical solutions that could be implemented to Bitcoin could be powered by renewable and sustainable stranded energy. Of course on the topic of energy use, the \cite{kar64} model or Energy Use function for civilizations might be a useful comparison point to contextualize given the potential of wide-use of Bitcoin adoption. In the \cite{kar64} equation 1:
