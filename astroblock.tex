...
Our theoretical and empirical framework is predicated upon the Kardashev model that attempts to define a `technological/cultural' state variable as a proxy over time, for the technological state of a 'typical' civilisation in The Universe. We first plot data for The Earth and then test the Kardashev model given the data. The variable is the "Energy Production" for which we use as an upper limit for consumption, and we assume an equal proportionality $\mathcal{E} = 1$ between 'production' and the amount of energy reserved for radio communication of scientific and cultural bearing or significance. We fix the percentage energy increase in production, year-over-year, to 1\%; for the standard Kardashev one percent model. We examine the Likelihood of the standard  model given the data for The Earth by comparing some statistical parameters. Perhaps not surprisingly in our analysis we reveal a potential contradiction for the original Kardashev model, given the data, and show the standard Kardashev model diverges. Occam's razor would indicate an alternate simple linear model with a Least-squares optimisation, our best fit working data model. However this linear model predicts an extrapolated Kardashev type II time-scale of an order of magnitude greater than the Age of The Universe, `The Conundrum'. A solution to `The Conundrum is proposed in the form of a new model from our working one: The Kardashev-Sagan-Nakamoto model, explored as fully as possible for the first time. The model  based on perhaps a foregone conclusion, that the standard Kardashev model being approximately exponential provides a divergent probability density distribution, and does not conserve its memory. The competing Kardashev-Sagan-Nakamoto model proposed to resolve the conundrum is constructed by transforming the data using a simple re-normalization or rescaling of the ordinate state variable. The total global produced power variable is renormalized by the Bitcoin hash rate year average in SI unites (Hash/Sec). This renormalisation yardstick is not massaging the data data but may actually be more closely related to a universal cosmological constant than does the original Kardasehv one-percent conjecture divergent model. This type of anthropic principled approach is consistent with the original intent of Kardashev to simultaneously include in the state-variable a measure of the security of a vast quantity of information for the civilisation that must be sensitive to the civilisation's technological prowess and energy production. 
