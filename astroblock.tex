...
The Kardashev law, one between energy use VS time for radio faring civilizations is examined and reconsidered with real data via an OLS fit, montecarlo experiment. The memoryless property that underpins the conventional constant percentage yearly increase or exponential growth-rate model is rejected for a linear model, one more consistent with data as revealed. To reconcile the Kardashev law with the Drake equation term of longevity for radio faring civilization, we propose a modified Kardashev relation by re-normalizing the original model for Bitcoin Hashrate. Our re-normalized model respects the original Kardashev timescales for Type-I, and Type-II as critical points for historical reasons. Moreover, following Kardashev and Sagan's suggestion of the importance of an extra information criterion to this law, we choose the Bitcoin hash-rate re-normalization as a natural way to include information transaction richness without including an extra parameter to this law, making it more general. A caveat to our analysis is that the energy data used represents total global Power production per year, however production is a valid upper-limit to consumption. Our modified Kardashev law is naturally more consistent with a much required interoperability development driver based on Open Public Blockchain Ledger technology for Astronomy \& Astrophysics Education, Research and Development from K-12 to tertiary-research level or more simply put for Astronomy Open Development (AOD). AOD as proposed is essentially a complimentary system of development through open governance of existing astronomical organizations and institutions that adhere to the open-source paradigm for collaborative scientific research development with adequate incentive tokenomics regulated through decentralized governance.
...
\section{Bitcoin and the Kardashev conundrum}
\label{sec:energy}

We now propose to use Bitcoin to reconcile the \cite{kar64} law with physical reality through a more realistic re-normalized power growth rate model than the 1\% model. We first observe the 1\% model fit fails to reproduce real data and violates the memory conservation property, that may actually be required for any technologically advanced civilization. The 1 \% model fit is shown in \ref{fig:kardashev1} with the data. The form of the fit is clearly inconsistent with the data and obviously produces unfathomable scatter about the data.
