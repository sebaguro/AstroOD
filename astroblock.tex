...
The Kardashev conjecture or hypothetical astrophysical law is one of the change of energy consumption (or Power VS Time) for radio-capable civilizations. We take global human Power production data from 1964 to 2022 and compare it to two Kardashev models. The standard and approximately exponential: "one-percent" per year monotonically increasing model, and, the Ordinary Least Squares fit linear data-model. The two competing models have significant different mathematical and statistical properties. The OLS linear model has a slope and intercept of $5.78 \pm 0.09$ TW per day and $-1.40  \pm 0.02 \times 10^{7} $ TW respectively, and the data-fit is uncanny.  The approximately exponential model on the other hand diverges from the data. The dimensional variables of power and time have time covariance and may naturally require further restriction; nothwithstanding this and against Julian Day as duly justified we show the linear data-model with its low dispersion across nearly two orders of magnitude in years, that transcends several technological and political eras of modern energy production from coal, oil, nuclear to renuables (hydro, wind, geothermal, solar etc.) has a Negative Log-Likelihood significantly smaller for as many parameters as does the standard model. The Hypothesis is that: The OLS model fit is not constrained by the \textbf{memoryless} probability property implicit in the standard model that precludes negative year-over-year growth, causing it to diverge and critically underestimate the lapse of time required for a typical nascent radio capable civilization to evolve to a Kard$\,_{\textrm{type-ii}}$ on the original scale, consuming as much energy per second as produced by a typical dwarf main sequence star, like The Sun. In leu of this result, that the original Kardashev scale is no longer tenable and to resolve a shortcoming of the linear model prediction: that radio-nascent civilizations may on-average require more than a Hubble time to evolve to Kard$\,_{\textrm{type-ii}}$; is that we propose the Kardashev-Nakamoto model to resolve this Conundrum by renormalizing the Power Production metric (J/s) by the Scalar Bitcoin Hash-Rate function year average (in Hash/s). The equivalence unit of J/Hash that results satisfies an implicit requirement; of accounting for information-richness without any additional parameters. Since the Bitcoin hash-rate is not a monotonically increasing function, with two-week difficulty adjustment then the the memory probability conservation property constraint is automatically resolved. We speculate on a prediction: if the Kardashev-Nakamoto model reflects physical reality, a handle on the Drake equation's longevity term for 'advanced' civilizations could be better modelled as well as the evolution of the Doomsday clock. The new Kardashev-Nakamoto straw-person model if approximately true provides a somewhat theoretical basis of an interoperability development driver for Astronomy \& Astrophysics; Education, Research and Development, based on Public Open Blockchain Ledger technology. A caveat to this analysis: the Power metric used is not derived from the "total global yearly average Energy consumed" but "total global yearly average Energy produced"; so for this purpose, production is an upper limit proxy to consumption for the Kardashev-Nakamoto model. 
