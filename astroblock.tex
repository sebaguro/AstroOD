...
The Kardashev astrophysical law is one of energy consumption versus time for radio-capable civilizations. We take global human Power production data from 1964 to 2021 and compare it to two Kardashev models. The standard Kardashev one-percent monotonically increasing model that is approximately exponential and, a linear model that is the OLS fit to the data. The two competing models have significant different mathematical and statistical properties. The OLS linear model has a slope and intercept of $0.6 \pm 0.1$ TW/yr and $-1136.3 \pm 18.5 $ TW respectively, and the data presents low dispersion about the fit. The axis variables of Power and Time for both models are the same, have time covariance and obviously must be further restricted; nothwithstanding this, the linear model's low dispersion across nearly two orders of magnitude in years that includes several technological eras (and political regulation) of energy production from Coal, Oil, Nuclear to Renuables (Hydro, wind, Geothermal, Solar etc.) appears to be more consistent with the maximal likelihood function of the Kardashev law for the data. Fundamentally it is not constrained by the \textbf{memoryless} probability property implicit in constant percentage increase models that preclude negative year-on-year growth and critically underestimate the lapse of time required for a typical nascent radio capable civilization to evolve to a type ii Kardashev one in consuming as much power as provided by a G-dwarf main sequence star, like The Sun. In leu of this severe limitation and because of the discrepancy of the linear model: that radio-capable civilizations may require more than a Hubble time to evolve into a Kardashev type-ii one; we consider the original Kardashev model is no longer tenable and so select the Kardashev OLS linear model fit and re-normalize it by the Hash-Rate of Bitcoin, defining a new Kardashev-Nakamoto model for the Kardashev law. The re-normalization is done by dividing the Power Production metric (J/s) by the Scalar Bitcoin Hash-Rate function (Hash/s). The equivalence unit of J/Hash that results might go in that vein, to satisfy an implicit requirement of the original Kardashev law that includes information-richness, obviating additional parameters. The Bitcoin hash-rate is a real valued scalar function with a two-week difficulty adjustment so the re-normalization may represent a more realistic Universal model for the Kardashev law. Incidentally the model selection choice if true may go someway to optimize the Drake equation's longevity term for radio-capable civilizations even justified by the advanced hour of the Doomsday clock. The new Kardashev-Nakamoto straw-person model provides a somewhat theoretical basis of an interoperability development driver for Astronomy \& Astrophysics; Education, Research and Development, based on Public Open Blockchain Ledger technology. A caveat to this analysis: the Power metric taken is not "yearly total global Energy consumed" but "yearly total global Energy produced"; so we assume production as an upper limit to consumption is a valid proxy for this study. 
...
Cryptography, the basis of OB ledger technology has until now not been much considered for astronomy development as revealed in 'full-text' searches of astronomical peer-reviewed international journal papers, least as a potential interoperability catalyzing agent. In fact cryptography for Astronomy has mainly been explored as an ancillary technology within data transfer applications like data compression, permission, (eg: ssh-rsa) and data fidelity procedures (eg: checksum).
