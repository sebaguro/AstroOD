...
This white paper examines Blockchain technology as a value optimization and interoperability agent for Astronomy Open Development (AOD). A comprehensive review of public and open Blockchain ledger technology including the evolution of Blockchain protocols: key ideas \& primitives leading to the  so-called Nakamoto consensus that effectively solves the Byzantine Generals problem defined by \cite{Lamport1982TheBG} is treated in  \cite{arvindandclark2017} and references therein. In Section \ref{sec:intro} a brief introduction is given on the history of the computer and computer systems as technological elements driven by astronomy and astrophysics, hereafter astronomy. An argument is made that identifies Blockchain technology as a potential corner-stone for the evolution of Astronomy Open (decentralized) development in Section \ref{sec:bc_review}, where an attempt is made to argue that the Nakamoto consensus of Blockchain is the natural evolution of the computer and computing systmes in which transactional value system for open development of astronomy may be preserved. In this section a short review of Blockchain  based on figure 1 of \cite{arvindandclark2017}) is given but additionally two key ideas are now herein made explicit: the Decentralized Finance and Decentralized Science Movements (DEFI and DESCI), as a fiducial family of parameters to our model, adapted schematically in Figure \ref{fig:narayanan}. Additionally in this section other open Blockchain protocols that may be useful for AOD are also considered for our model (straw-person).  In Section \ref{sec:energy} by taking the time constraints from the original Kardashev relation, of the time required for Earth's civilization to reach type I, type II and type III status,  we take the Nakamoto consensus and build it into our model by rescaling the Kardashev function by the Bitcoin hash-rate. We base this decision by demonstrating issues with the current Kardashev models, and compare the original and re-fitted Kardashev scales in this analysis. Our modified Kardashev scale, normalized by the Bitcoin hash-rate includes the assumption that the Bitcoin (Captial `B') as a network and bitcoin (Lower-case `b') the commodity combined, represent a sound store-of-vale transaction base-layer protocol for AOD, one that preserves information-richness as necessarily identified by Kardeshev in his original 64' paper and subsequent works. In Section 4 specific use-cases of Blockchain AOD are identified and examined and finally future prospects and discussion are addressed in Section 5 and Section 6, respectively.
...
\begin{figure}
    \centering
    \label{fig:astropyhttps://www.overleaf.com/project/5e6a5fbf3f87b80001235c1b}
...
\begin{figure}
    \centering
    \label{fig:ai}
    \includegraphics[width=0.38\textwidth]{figs/fulltextai.png}
    \vspace*{-0.3cm}
    \caption{Showing Full-text search for  full:"Artificial" and full:"inteligence" or full: "AI" or full: "machine learning")) AND year:1976-2022) returned query in NASA ADS}
\end{figure}
